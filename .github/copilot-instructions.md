...
## Critical "Gotchas" Fixed Recently

### Key Vault Access Policy (RESOLVED - Nov 2025)
**Problem**: Admin dashboard failed with "Unauthorized" because `ADMIN_API_KEY` Key Vault reference wasn't resolving. App Service needs explicit access policy to read secrets.

**Root Cause**: Terraform creates the access policy, but timing issues or incomplete applies can leave App Service without permissions.

**Fix Pattern**:
```bash
# Check if App Service has Key Vault access
az webapp identity show --name <app-name> --resource-group <rg> -o json | jq -r '.principalId'
az keyvault show --name <kv-name> --resource-group <rg> --query "properties.accessPolicies[?objectId=='<principal-id>']"

# Add access policy if missing
az keyvault set-policy --name <kv-name> --resource-group <rg> \
  --object-id <principal-id> --secret-permissions get list

# CRITICAL: Restart App Service to refresh Key Vault references
az webapp restart --name <app-name> --resource-group <rg>
```

**Prevention**: GitHub Actions workflow (`.github/workflows/azure-deploy.yml`) now includes restart step after deployment.
...
# REQUIRED for admin dashboard (generated by Terraform, stored in Key Vault)
ADMIN_API_KEY=<32-char-random-password>
...
### Admin Dashboard Authentication (`app/api/admin/results/route.ts`)
- Protected by simple API key auth (header: `x-admin-api-key` or `Authorization: Bearer`)
- Key stored in Key Vault, referenced in App Service app settings
- Client-side prompt in `app/admin/page.tsx` asks for key on load
- Get key: `az keyvault secret show --vault-name <kv-name> --name admin-api-key --query value -o tsv`

### Rate Limiting Pattern (`app/api/analyze/route.ts`)
- **In-memory token bucket** per IP address (50 requests/minute)
- Not distributed - each App Service instance has its own limits
- Production should use Redis or Azure API Management for distributed rate limiting
- Pattern: Check bucket → consume token → process request → return 429 if exhausted
...
- **Removes package.json from deploy folder** to prevent Kudu from running npm install
- Deploys via `az webapp deploy --type zip` (90s timeout)
- **Restarts App Service** to refresh Key Vault references (critical for ADMIN_API_KEY)
- Uses `startup.sh` script to launch standalone server

**Standalone Build Details** (`next.config.ts: output: 'standalone'`):
- Next.js generates minimal server.js + dependencies in `.next/standalone/`
- Must copy `.next/static/` and `public/` separately into deployment
- startup.sh runs `node server.js` (NOT `npm start`)
- Startup command configured via `az webapp config set --startup-file "bash startup.sh"`
...
10. **Key Vault References**: App Service must restart after Terraform apply to load Key Vault references - see "Gotchas" section
...
- Key Vault: All secrets stored here, App Service accesses via managed identity and access policy
- App Service: Linux B1, Node.js 20 LTS, standalone output mode, system-assigned managed identity

**Post-Terraform Steps**:
1. Verify App Service has Key Vault access policy (see "Gotchas" section if admin dashboard fails)
2. Restart App Service to load Key Vault references: `az webapp restart --name <app-name> --resource-group <rg>`
3. Test admin endpoint with key from Key Vault
